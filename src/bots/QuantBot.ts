...
import { OrderBatcher } from "./OrderBatcher.js";

// Re-export OrderBatcher for backward compatibility
export { OrderBatcher } from "./OrderBatcher.js";
...
export class TradeOrder {
  ...
  side: Side;
  isAudited: boolean;
  periodId: number;  // Period ID when order was created
  ...
  constructor(props: TradeOrderProps) {
    ...
    this.periodId = props.periodId ?? 0;
  }
...
export class QuantBot {
    ...
    const orderPromise = (async (): Promise<TradeOrder | undefined> => {
      ...
      try {
        ...
        const  trade = new TradeOrder({
          ...
          periodId: this.currentPeriodId,  // Store period for cross-period validation
        });
  ...
  public async onSimulationPeriodEnd(): Promise<TradeOrder[]> {
    ...
    // This MUST happen before updateOrders() to prevent cross-period matching
    ...
    // Update orders after periodId increment - orders from previous period won't match
  ...
  private async updateTestOrder(trade: TradeOrder): Promise<void> {
    ...
    // Validate order is from current period (prevents cross-period matching)
    // Orders created in a previous period should not match in the current period
    if (trade.periodId !== this.currentPeriodId) {
      // Order is from a different period - don't allow matching
      return;
    }

    // Use midpoint price for more realistic order matching
    // getMidPrice returns null if the token ID doesn't match the current period's tokens
    const midPrice = await this.marketInfo.getMidPrice(trade.clobTokenId, this.targetedMarket);

    // If price is null, the token is from a different period - don't match
    if (midPrice === null) {
      return;
    }
  ...
  private settleExpiredPositions(
    ...
    for (const [clobId, amount] of Object.entries(positionsByClob)) {
      ...
      // For formatted IDs: "UP-2026-0-23-20" or "DOWN-2026-0-23-20"
      // For raw numeric IDs: use the allClobTokenIds array (index 0 = UP, index 1 = DOWN)
      let positionDirection: 'UP' | 'DOWN';
      if (clobId.startsWith('UP-')) {
        positionDirection = 'UP';
      } else if (clobId.startsWith('DOWN-')) {
        positionDirection = 'DOWN';
      } else {
        // Raw numeric token ID - use the allClobTokenIds array to determine direction
        positionDirection = clobId === allClobTokenIds[0] ? 'UP' : 'DOWN';
      }
      const isWin = assetPriceWinner !== null && positionDirection === assetPriceWinner;
      const finalValue = isWin ? amount : 0;

      this.writeLog(`Settlement: clobId=...${clobId.slice(-20)}, positionDir=${positionDirection}, assetWinner=${assetPriceWinner}, isWin=${isWin}`);
      this.writeLog(`...${clobId.slice(-20)} expired (${isWin ? 'win' : 'loss'}) with ${amount} units for $${finalValue}`);
