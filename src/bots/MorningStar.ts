...
interface MorningStarProps extends QuantBotProps {
    candleMinutes: number;          // Duration of each candle (e.g., 10 minutes)
    minBearishMove: number;         // Min price drop for first candle to be "bearish" (e.g., 50 = $50)
...
type TradingState =
    | 'FORMING_CANDLE_1'    // First candle is forming (looking for bearish)
    | 'FORMING_CANDLE_2'    // Second candle is forming (looking for indecision)
    | 'FORMING_CANDLE_3'    // Third candle is forming (looking for bullish)
    | 'PATTERN_DETECTED'    // Morning Star pattern detected, ready to trade
...
export class MorningStar extends QuantBot implements QuantBotRun {
    ...
    constructor(props: MorningStarProps) {
        super(props);
    ...
    // -------------------------------------------------------------------------
    // Candle Evaluation - Morning Star Pattern
    // -------------------------------------------------------------------------

    /**
     * First Candle (Bearish): A long, solid bearish candle indicating strong
     * selling pressure and continuation of the downtrend.
     */
    private evaluateFirstCandle(): void {
        ...
        if (isBearish) {
            ...
            this.writeLog(`Candle 1 (Bearish) formed: Open=${this.candle1.open.toFixed(2)}, Close=${this.candle1.close.toFixed(2)}, Change=${priceChange.toFixed(2)}`);
        } else {
            this.writeLog(`Candle 1 not bearish enough: Change=${priceChange.toFixed(2)}, Required=${-this.minBearishMove}`);
            // Stay in FORMING_CANDLE_1, next candle might be bearish
        }
    }

    /**
     * Second Candle (Indecision): A small body (like a Doji or spinning top),
     * often gapping down, showing market pause and uncertainty.
     */
    private evaluateSecondCandle(): void {
        ...
        // Check for gap down (open below previous close)
        const hasGapDown = this.currentCandle.open < this.candle1.close;

        if (isIndecision) {
            this.candle2 = { ...this.currentCandle };
            this.state = 'FORMING_CANDLE_3';
            this.writeLog(`Candle 2 (Indecision) formed: Range=${candleRange.toFixed(2)}, Body=${bodySize.toFixed(2)}, GapDown=${hasGapDown}`);
        } else {
    ...
    /**
     * Third Candle (Bullish): A strong bullish candle that closes significantly
     * into the first candle's body (above its midpoint), signaling buyers are taking over.
     */
    private evaluateThirdCandle(): void {
        ...
        // Check if close is above midpoint of first candle
        const firstCandleMidpoint = (this.candle1.open + this.candle1.close) / 2;
        const closesAboveMidpoint = this.currentCandle.close > firstCandleMidpoint;

        if (isBullish && closesAboveMidpoint) {
            this.candle3 = { ...this.currentCandle };
            this.state = 'PATTERN_DETECTED';
            this.writeLog(`Morning Star Pattern DETECTED!`);
            ...
            this.writeLog(`Candle 3 not bullish enough: Change=${priceChange.toFixed(2)}, Required=${this.minBullishMove}, ClosesAboveMidpoint=${closesAboveMidpoint}`);
            // Reset to look for new pattern
            this.resetPatternSearch();
        }
    }
